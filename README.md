...
- Setting "**Replicate database to indexers**" as **YES** will enable replication on MMDB bundle and also make bunch of changes in the code that will enable _ipinfo_ to work in streaming more. This is expected to cause performance boost on the query at the expense on increase in bundle size. This setting is applicable if you using ipinfo app on splunk search head cluster and you have indexer cluster.
...
### `ipinfo` (resproxy with 7 days lookback)
...
### `ipinforesproxy` (with 7 days lookback)
